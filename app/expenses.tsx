...
export default function ExpensesScreen({ navigation }: any) {
  ...
  const handleSave = async () => {
    ...
    console.log('[expense insert] error:', error, 'data:', data);
    ...
    if (!data || data.length === 0) {
      Alert.alert(
        'บันทึกไม่สำเร็จ',
        'Insert ไม่ได้คืนข้อมูลกลับมา — น่าจะติด RLS policy ของตาราง transactions_tb ใน Supabase'
      );
      return;
    }
      ...
      <View style={formStyles.topBanner}>
        <View>
          <View style={formStyles.profileRow}>
            <Text style={formStyles.profileName}>Rapeepat theampol</Text>
